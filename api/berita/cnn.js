...
module.exports = function (app) {
  app.get("/berita/cnn", async (req, res) => {
    ...
    try {
      const response = await axios.get("http://ikyyzyyrestapi.my.id/berita/cnn", {
        headers: {
      ...
      console.error("[CNN INDONESIA NEWS]", error.message);
      
      return res.status(500).json({
        status: false,
        creator: "ibnu",
        message: "Gagal mengambil berita dari CNN Indonesia"
      });
    }
  });
};